...
OAuth.prototype.makeAuthenticatedRequest = function makeAuthenticatedRequest(command,options){
	self = this;
	return new Promise(function(resolve,reject) {
	    ...
	    if (err) {
	      ...
	      self.makeRequest(command, options, function(requestErr,response){
					if(requestErr){
						reject(new Error(requestErr))
					} else {
						resolve(response);
					}
				});
